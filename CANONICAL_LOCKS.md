...
5. **TL otoritesi:** Açık IMS döneminde TL hedef ve TL gerçekleşen resmi IMS kaynağından; P1/P2 geldiğinde resmi üretim kaynağından alınır. Kutu/bakiye alanlarından TL türetilmez.
6. **Temsilci rekabet tutarlılığı:** Ürün bazlı rakip toplamı, seçili ürün rakip detayları, toplam kutu pazarı, pazar payı ve aylık rakip değişimi aynı DB veri zincirini kullanır. Önceki ay verisi önceki ayın kendi temsilci-brick kapsamından okunur.
7. **Sayı gösterimi:** Kutu adetlerinde Türkçe binlik ayırıcı `.` kullanılır; ör. `9.360 kutu` = dokuz bin üç yüz altmış. Kutu farkı ve yüzde değişimi ayrı anlamlarla gösterilir.
8. **Bölge/temsilci/Türkiye tutarlılığı:** Aynı iş kuralını kullanan temsilci, bölge ve Türkiye ekranları birbirinden bağımsız alternatif formül üretmez. Bir kaynak/formül değişikliği ilgili tüm tüketici ekranların regresyon testinden geçmeden tamamlanmış sayılmaz.
9. **Özel kimlik kuralları:** `BOS` ve `BOŞ` ayrı kimliklerdir; `BOSTANCI` normal değerdir. Gerçek sayısal `0` boş/verisiz sayılamaz.
10. **Yetki ve güvenlik:** Bölge müdürü kapsamı fail-closed kalır; yetkisiz bölge/temsilci erişimi açılamaz. Admin/özel yetki kuralları mevcut merkezi yetki servisinden okunur.
11. **Dağıtım güvenliği:** İlgili CI PASS olmadan merge yoktur. Production acceptance PASS olmadan tamamlandı denmez. Aktif `PROCESSING` IMS import işi varken deploy/restart yapılmaz. SQLite `WAL` ve `busy_timeout=30000` korunur.
