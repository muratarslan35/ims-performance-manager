...
def main() -> int:
    ...
    with app.app_context():
        ...
        expected_ims_id, expected_production_id = PersistentDashboardSnapshotService.source_identity(
            year, month
        )
        ...
        for _ in range(max(1, args.reads)):
            ...
            if not isinstance(ready, dict) or not ready:
                print(
                    "DASHBOARD_SNAPSHOT_ACCEPTANCE|status=FAIL|reason=became_unavailable|"
                    f"year={year}|month={month}|ims_upload_id={expected_ims_id}|"
        ...
        actual_ims_id, actual_production_id = PersistentDashboardSnapshotService.source_identity(
            year, month
        )
        ...
        if (actual_ims_id, actual_production_id) != (expected_ims_id, expected_production_id):
            print(
                "DASHBOARD_SNAPSHOT_ACCEPTANCE|status=FAIL|reason=source_changed|"
                f"year={year}|month={month}|expected_ims_upload_id={expected_ims_id}|"
                f"actual_ims_upload_id={actual_ims_id}|expected_production_upload_id={expected_production_id}|"
                f"actual_production_upload_id={actual_production_id}"
            )
            return 1
        if p95 > args.max_read_seconds:
            print(
                "DASHBOARD_SNAPSHOT_ACCEPTANCE|status=FAIL|reason=slow_read|"
                f"year={year}|month={month}|ims_upload_id={actual_ims_id}|"
                f"production_upload_id={actual_production_id}|reads={len(read_times)}|"
                f"read_p95_seconds={p95:.4f}|read_mean_seconds={mean:.4f}|"
                f"max_read_seconds={args.max_read_seconds:.4f}"
            )
            return 1

        print(
            "DASHBOARD_SNAPSHOT_ACCEPTANCE|status=PASS|"
            f"year={year}|month={month}|ims_upload_id={actual_ims_id}|"
            f"production_upload_id={actual_production_id}|reads={len(read_times)}|"
            f"read_p95_seconds={p95:.4f}|read_mean_seconds={mean:.4f}|attempts={attempts}"
        )
        return 0
