...
from app.models import Representative, Product
...
def build_overrides(data):
    ...
    for item in data.get("products", []):
        ...
        overrides[product_id] = {
            "unit": max(0, float(item.get("unit", 0))),
            "tl": max(0, float(item.get("tl", 0)))
        }
...
def index():
    representatives = (
        ...
        .order_by(Representative.rep_name.asc())
        .all()
    )
...
def calculate():
    try:
        ...
        representative_id = int(data.get("representative_id", 0))
        year = int(data.get("year", 0))
        month = int(data.get("month", 0))

        if representative_id <= 0:
            return jsonify(success=False, message="Temsilci seçiniz."), 400

        if year <= 0:
            return jsonify(success=False, message="Yıl bilgisi eksik."), 400

        if month < 1 or month > 12:
            return jsonify(success=False, message="Geçersiz ay."), 400
        ...
        if duplicates:
            return jsonify(
                success=False,
                message="Aynı ürün birden fazla gönderildi.",
