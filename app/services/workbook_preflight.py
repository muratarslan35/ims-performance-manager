...
class WorkbookPreflight:
    ...
    def _header_signature(self,frame):
        values=[]
        # Scan a wider vertical window so moved/merged headers remain discoverable.
    ...
    def _content_classify(self,frame):
        sig=self._header_signature(frame)
        tokens=set(re.findall(r"[A-Z0-9ÇĞİÖŞÜ]+",sig))
        has=lambda *names: all(self._normalized(name) in sig for name in names)
        # Strong semantic signatures; independent of sheet title/order.
        ...
        # Generic representative header discovery is deliberately structural.
    ...
    def build(self):
        ...
        for sheet_name,frame in (self.service.workbook or {}).items():
            ...
            manifest.append({"sheet_name":str(sheet_name),"rows":int(frame.shape[0]),"columns":int(frame.shape[1]),"meaningful_cells":meaningful,"header_row":header_row,"sheet_type":sheet_type,"coverage":coverage,"classification_basis":"content" if not explicit_nondata and self._content_classify(frame)==sheet_type else ("nondata" if explicit_nondata else "name_fallback")})
        return manifest
    ...
    def validate(self):
        ...
        ledger=self.build_cell_ledger(manifest); unclassified_cells=[cell for cell in ledger if cell["classification"] not in self.TERMINAL_CELL_CLASSES]
