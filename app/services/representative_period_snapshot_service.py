...
class RepresentativePeriodSnapshotService:
    ...
    def build(cls, representative_id, year, month):
        ...
        if not targets:
            return {
                key: {
                    "key": key, "label": label, "month_count": length,
                    "target_tl": Decimal("0"), "actual_tl": None,
                    "realization_percent": None, "gap_tl": None,
                    "complete": False, "products": [], "representatives": [],
                }
        ...
        uploads = ProductionResultUpload.query.filter(
            ...
            ProductionResultUpload.year.asc(), ProductionResultUpload.month.asc(),
            ProductionResultUpload.production_stage.desc(),
            ProductionResultUpload.applied_at.desc(), ProductionResultUpload.id.desc(),
        ).all()
        ...
        for target in targets:
            ...
            for upload in uploads_by_period.get(period, ()):
        ...
        for key, label, length in cls.PERIODS:
            ...
            for product_id, values in product_totals.items():
                ...
                product_rows.append({
                    ...
                    "gap_tl": values["target"] - values["actual"] if complete else None,
                    "complete": complete,
                })
