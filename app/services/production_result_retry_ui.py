...
def install_production_result_retry_ui(app):
    """Add safe in-place retry for failed production-result audit rows.

    Failed workbooks are already retained in the protected production staging
    directory. Retrying reuses that exact source, verifies its SHA-256 against
    the audit row, and applies it only if the adaptive parser validates it.
    No new upload row or duplicate source hash is created.
    """
